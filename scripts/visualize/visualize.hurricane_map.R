...
visualize_hurricane_map <- function(viz, height, width, mode, ...){
  ...
  xml_add_child(g.overlays, 'text', "TEXAS", class='svg-text state-name', id="texas", transform="translate(150,140)", 'text-anchor'='middle')
  ...
  for (i in seq(n.bins, 1, -1)){
    ...
    text.class <- ifelse(any(col2rgb(colors[i]) < 130), 'svg-text light-rain-legend', 'svg-text dark-rain-legend')
  ...
  ys <- seq(35, height - 60, length.out = nrow(sparks))
  ...
  g.watermark <- xml_add_child(non.geo.top, 'g', id='usgs-watermark',
                               transform=sprintf('translate(%s,%s)scale(0.2)', 
  ...
  storm.states <- storm.states[!storm.states %in% "Mexico"]
